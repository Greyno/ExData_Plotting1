...
##Print to the screen device using plot (hit the refresh button in the plot window if needed)


##Plot graph #4 to a file - set the size to 480x480 pixels
png(file="plot4.png", width=480, height=480)
par(mfcol=c(2,2)) #Specify 2 rows, 2 col; plotted rowwise (l-r, top to bottom)
...
#Use bty="n" to not put a box around the legend
legend("topright", lty=1, col=c("black", "red", "blue"),bty="n", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

plot(subset_data$Voltage~subset_data$Datetime, type="l", ylab="Voltage",xlab="datetime")

plot(subset_data$Global_reactive_power~subset_data$Datetime, type="l", ylab="Global_reactive_power",xlab="datetime")
dev.off()